export async function agent(sentence: string, word: string) {
  ...
  const meaning = await matchMeaning(sentence, word, reduceToken(meanings))
  const [partOfSpeech, definition] = meaning.split('\n')
  return { sentence, word, partOfSpeech, definition }
}

async function fetchMeanings(word: string) {
...
async function matchMeaning(
  ...
  Output Format:
  {partOfSpeech}
  {selected definition} (exact wording from JSON)
